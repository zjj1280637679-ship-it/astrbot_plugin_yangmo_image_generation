...
- 编辑当前消息直接上传的图片，或编辑用户明确回复/引用的图片时，都优先使用 `refs=["current"]`。插件会先读取当前直接图片，再尝试引用图快速路径；只有这个快速路径失败时，才需要 Agent 调外部图片定位/解析工具并改用 `resolved`。
- 继续编辑历史生成物时使用对应 `genimg:` 引用。
- 不要为了遵守流程而增加工具调用。任务足够明确时直接生成；只有能力、模型、画幅、参考图边界或交付语义不确定时才查询 `list_image_capabilities`。

## 引用 / 回复图片的最短路径

当用户在 QQ 中回复一张旧图片并要求“按这个改”“把这个做成视频”等，不要默认先调用历史图片搜索工具。

`current` 的图片解析顺序是：

1. 当前消息直接附带的图片；
2. `Reply.chain` 中已经随事件附带的被引用图片（零额外 OneBot 查询）；
3. 若 Reply 中没有原图，调用 AstrBot 自带 quoted-message parser / OneBot resolver 做一次有超时上限的快速取图；
4. 快速路径失败或超时，才回退到现有 Agent 图片定位/搜索工具，再使用 `resolved`。

这个快速路径不调用图片描述模型，所以群聊图片理解模型的 400、格式错误或 contentFilter 不应阻断真正的原图参考链路。引用解析失败也不应被当作生图 API 错误；它只是意味着需要使用下一层兜底。

如果用户同时“直接上传新图 + 回复旧图”，直接上传图保持更高优先级，尤其是视频 `first_frame=current` 时不会让旧引用图悄悄覆盖新上传图。
...
`refs=["current"]` 既可用于当前消息直接图片，也可用于明确的 QQ 回复/引用图片。引用图会先走本插件对 AstrBot 原生 quoted-message 解析能力的快速复用，不需要 Agent 先人为执行一次图片抓取。
